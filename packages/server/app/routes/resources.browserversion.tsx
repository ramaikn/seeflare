...
export async function loader({ context, request }: LoaderFunctionArgs) {
    ...
    const fetchData = async () => {
        const countsByProperty = await unifiedQuery.getCountByBrowserVersion(
            site,
            interval,
            tz,
            filters,
    ...
    try {
        if (isExtended) {
            ...
            const cacheKey = buildCacheKey("browserversion", {
                site,
                interval,
                tz,
                page: pageNum,
        ...
        console.error("browserversion loader error:", error);
