...
const routes = [
    ...
    { file: 'resources.device.tsx', method: 'getCountByDeviceType', cacheKey: 'device' },
...
routes.forEach(({ file, method, cacheKey }) => {
    ...
    if (!content.includes('import { isExtendedInterval }')) {
    ...
    // Replace the loader function entirely
    // We can just find the export const ComponentCard and replace everything before it with the new loader (except imports)
...
`;
    ...
    // Find the end of imports (first empty line after imports or start of export async function loader)
    const loaderStart = content.indexOf('export async function loader');
    const exportCardMatch = content.match(/export const [A-Za-z]+Card/);
    if (loaderStart !== -1 && exportCardMatch) {
        const cardStart = exportCardMatch.index;
        content = content.substring(0, loaderStart) + newLoader + '\n' + content.substring(cardStart);
